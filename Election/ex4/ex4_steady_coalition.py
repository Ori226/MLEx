...
if __name__ == "__main__":
    ...
    for i in range(6):
        clustering_model = KMeans(init='k-means++', n_clusters=i+2, n_init=10)
        clustering_model.fit(train.data)
        k_means_prediction = clustering_model.predict(train.data)
        plt.hist(k_means_prediction)
    plt.show()

    # clustering_model = KMeans(init='k-means++', n_clusters=3, n_init=10)
    # clustering_model.fit(train.data)
    # k_means_prediction = clustering_model.predict(train.data)
    ...
    # a cluster represent a homegenouse feature space
    # the smallest area where there are more than 51%. In this area, even if the voters will be manipulated, the

    # after training the k-means , find the cluster with the largest
